...
class ContextMethods {
    ...
    static prefs(Activity self, int mode = Context.MODE_PRIVATE, @DelegatesTo(SharedPreferences.Editor) Closure sharedPreferencesSpec){
        SharedPreferences.Editor editor = prefs(self, mode).edit()
        editor.with(sharedPreferencesSpec)
        editor.commit()
    }
    static SharedPreferences prefs(Activity self, int mode = Context.MODE_PRIVATE){
